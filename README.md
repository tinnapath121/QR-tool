...
2. **Google AdSense — current status: applied, awaiting approval**
   ...
   - `loadAdsense()` in `js/app.js` (section 9) injects the AdSense script **unconditionally on every page load** — it is NOT gated behind the cookie banner. This is deliberate: outside the EEA/UK/Switzerland, Google doesn't require consent to serve ads at all, and inside those regions Google's own systems automatically restrict ads to non-personalized when no certified Consent Management Platform is detected (this site doesn't have one). Gating all ads behind an "Accept" click would be stricter than Google requires and would just cost impressions.
   - Cookie consent (`initCookieConsent()`) still exists and still gates **Google Analytics** only (`loadAnalyticsIfConsented()`) — that distinction is intentional.
   ...
   - If you later want **personalized** ads (higher RPM) for EEA/UK/CH visitors specifically, that requires integrating a Google-certified CMP with full Consent Mode v2 — a bigger change than what's here now; ask if you want it added.
